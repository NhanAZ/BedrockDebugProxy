...
type Dialer struct {
	...
	// ClientGUID is the RakNet client identifier used during connection setup.
	// If zero, a process-local random identifier is generated. Reusing this
	// value is useful when following a server Transfer on the same client.
	ClientGUID int64
...
func (dialer Dialer) PingContext(ctx context.Context, address string) (response []byte, err error) {
	...
	data, _ := (&message.UnconnectedPing{PingTime: timestamp(), ClientGUID: dialer.clientGUID()}).MarshalBinary()
...
func (dialer Dialer) clientGUID() int64 {
	if dialer.ClientGUID != 0 {
		return dialer.ClientGUID
	}
	return atomic.AddInt64(&dialerID, 1)
}
...
func (state *connState) discoverMTU(ctx context.Context) error {
	...
	for {
		...
		case message.IDOpenConnectionReply1:
			...
			state.serverSecurity = response.ServerHasSecurity
			state.cookie.Store(response.Cookie)
...
func (state *connState) openConnection(ctx context.Context) error {
	...
	state.request2MTU.Store(uint32(state.mtu))
	...
	for {
		...
		switch b[0] {
		case message.IDOpenConnectionReply1:
			// A flow-hashed frontend may rotate its anti-spoof cookie after the
			// first Request2. Refresh the cookie and retry Request2 on the same
			// UDP flow instead of waiting for the stale cookie to time out.
			response := &message.OpenConnectionReply1{}
			if err := response.UnmarshalBinary(b[1:n]); err != nil {
				return fmt.Errorf("read refreshed open connection reply 1: %w", err)
			}
			if response.ServerGUID == 0 || response.MTU < 400 || response.MTU > 1500 {
				continue
			}
			if response.ServerHasSecurity == state.serverSecurity {
				state.cookie.Store(response.Cookie)
				state.request2MTU.Store(uint32(response.MTU))
				state.openConnectionRequest2(response.MTU)
			}
...
func (state *connState) request2(ctx context.Context) {
	...
	requestCount := 0
	for {
		mtu := uint16(state.request2MTU.Load())
		state.openConnectionRequest2(mtu)
		requestCount++
		if requestCount%4 == 0 {
			// Re-requesting Reply1 periodically lets a flow-hashed edge refresh
			// its cookie without opening a second UDP socket.
			state.openConnectionRequest1(mtu)
		}
		select {
		case <-state.ticker.C:
			continue
