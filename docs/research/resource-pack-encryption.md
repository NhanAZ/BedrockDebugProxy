...
| [CloudburstMC/ProxyPass](https://github.com/CloudburstMC/ProxyPass/tree/baa6d9a565c58f5f2306c5646fc833704253109f) | `baa6d9a565c58f5f2306c5646fc833704253109f`, AGPL-3.0 | [`PackDownloader.java`](https://github.com/CloudburstMC/ProxyPass/blob/baa6d9a565c58f5f2306c5646fc833704253109f/src/main/java/org/cloudburstmc/proxypass/network/bedrock/session/PackDownloader.java) corroborates `AES/CFB8/NoPadding`, the first 16 key bytes as IV, the 256-byte prefix, and per-file keys. It was used only as behavioral research. |
| [NIST CAVP block-cipher validation](https://csrc.nist.gov/projects/cryptographic-algorithm-validation-program/block-ciphers) | Official validation material | The `CFB8GFSbox256.rsp` known-answer vector from NIST's AES KAT archive validates the local CFB8 state update independently of the Bedrock implementations. |
...
Real Minecraft validation is still required. Test an owner-controlled server with no pack, an unencrypted pack, an encrypted pack, encrypted subpacks, multiple sequential packs, RakNet delivery, and an advertised HTTP URL. The current gophertunnel upstream path is expected to request chunks over RakNet rather than dereference that URL, so retain the raw `ResourcePacksInfo` as evidence. A successful synthetic test is not evidence that every live server uses the supported variant.
