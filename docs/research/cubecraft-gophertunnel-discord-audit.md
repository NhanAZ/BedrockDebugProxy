...
## September 2026 signaling and identity note

The maintainer-provided Discord excerpt from September 2026 contains a new but unverified observation that CubeCraft appears to expose an HTTP signaling path while returning `404` from `/v1/join`. The same discussion first reported `mc-raknet-allowed = false`, then corrected that value to `true` and identified the false value as a bot mistake. Treat the corrected RakNet value as a community status observation, not as an immutable server guarantee.

The current [Mojang NetherNet HTTP signaling guide](https://mojang.github.io/bedrock-protocol-docs/guides/nether-net-onboarding-guide/) says that `GET /v1/join` is the capability check, that a `2xx` response indicates NetherNet support, and that a non-`2xx` response such as `404` means the client will not attempt WebRTC. It also separates that check from the SDP exchange at `POST /v1/join/{networkId}`. The guide is generated for a newer protocol page than this project's 1.26.50 baseline, so it is used here only to interpret the endpoint semantics. The Discord observation does not establish that CubeCraft's production client path supports NetherNet, nor that it is related to Sentinel.

The attached screenshot shows a decoded JWT header with `alg: ES384` and a payload containing `iss: "CubeCraft Dev"`, timestamps, and a `cpk` claim. The `cpk` value is intentionally not copied into this repository. Mojang's guide describes `cpk` as the public key bound to the identity assertion and explains that identity assertions bind a token to a WebRTC DTLS fingerprint. This is relevant to a future NetherNet implementation, but it is not evidence that the current RakNet proxy path can or should reuse that token. It also does not reveal which component signed the screenshot's token or whether it came from a production connection.

No BedrockDebugProxy change follows from this excerpt. CubeCraft remains excluded from live validation and release gates under the project's existing policy. The current proxy uses the RakNet path, and adding NetherNet signaling would be a separate transport and identity project requiring explicit scope and authorized test infrastructure.
