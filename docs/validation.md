...
Normal connection termination produces read-side `transport.payload` and `bridge.read_error` evidence when a read loop ends. After one forwarding loop ends, the proxy marks coordinated shutdown before closing the peer connection. A peer loop that is awakened by that close may return `context.Canceled` or `net.ErrClosed`; those expected bridge errors remain in the `session.close` diagnostics but are not emitted as `bridge.*_error` events. Errors that occur before coordinated shutdown remain recorded. The report preserves termination counts without automatically treating them as protocol failures. The developer must still inspect them and may mark the session `fail` when their timing or cause is abnormal. Transport writes that fail before coordinated shutdown, library errors, resource-pack derivation errors, and all other error kinds block an automatic pass.
