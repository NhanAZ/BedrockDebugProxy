...
Normal connection termination produces read-side `transport.payload` and `bridge.read_error` evidence when a read loop ends. The first forwarding loop to terminate claims coordinated shutdown before its error is classified, so the peer loop cannot race the shutdown marker. A peer loop that is awakened by that close may return `context.Canceled` or `net.ErrClosed`; those expected bridge errors remain in the `session.close` diagnostics but are not emitted as `bridge.*_error` events. A first write-side close error can be the loop that observes the disconnect, so `bridge.write_error` and `bridge.flush_error` with those transport errors are treated as the shutdown signal and omitted as well. Other writes before a close, library errors, resource-pack derivation errors, and all other error kinds remain recorded and block an automatic pass. The report preserves termination counts without automatically treating them as protocol failures. The developer must still inspect them and may mark the session `fail` when their timing or cause is abnormal.
