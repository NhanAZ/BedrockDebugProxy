...
only documentation adds cost without testing a changed runtime. Editing a report's `tested_revision` would be
...
files, `docs/`, and the release-readiness checker itself. Any product code, other tool, workflow, configuration,
generated, or other non-documentation change requires a fresh exact-revision matrix. The checker change is
limited to this evidence-selection policy and remains subject to automated fail-closed tests.
...
- Existing live evidence remains truthful and reusable for a strictly documentation and release-checker-only
  release delta.
- The validation command has an explicit `-ValidatedRevision` input and fails closed when the ancestry or path
  allowlist check does not pass. The only non-Markdown path permitted in the allowlist is this checker.
- Documentation-only changes must still be reviewed, pass automated quality and CI checks, and be recorded in an
  audit entry.
- Runtime, protocol, transport, capture, authentication, other tooling, workflow, or generated-file changes cannot
  use this exception.

## Evidence and validation

- The candidate delta from `2a94412e11139184cc2b949dca75b18d0f4048e5` to the current release preparation commit
  contains only Markdown files and the release checker.
