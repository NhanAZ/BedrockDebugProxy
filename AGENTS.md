...
Public issue and pull request comments are maintainer communication. When the maintainer explicitly authorizes a comment from the maintainer account, write it in a natural, respectful first-person maintainer voice and state only evidence that was actually reviewed. Do not present an automated agent as a separate human participant or claim personal actions that did not occur. For bot-authored dependency pull requests such as Dependabot updates, keep routine review comments short and explain the decision with one concrete reason. Do not anthropomorphize the bot with greetings, thanks, or other social language intended for a human contributor. Add more detail only when it is needed for audit or debugging. Prefer complete sentences over label-style fragments such as `Status: explanation`, and replace semicolons with a period or `and` when that reads more naturally. Proofread the final public text before posting.
...
The scheduled Mojang protocol watcher may open a deduplicated issue when an official `bedrock-protocol-docs` release differs from the project baseline. Treat that issue as a report to triage, not an implementation order. Verify the release tag commit and schemas, record evidence under `docs/research/`, and follow [`docs/protocol-updates.md`](docs/protocol-updates.md) before changing protocol code.
...
Every official release requires reports tied to a validated runtime revision for The Hive, Galaxite, Lifeboat, Mineville Zeqa, and Enchanted. A later candidate may reuse those reports only through the explicit docs-only equivalence in `docs/validation.md`, which verifies ancestry and an allowlist containing only Markdown documentation. Never edit a report's `tested_revision`. Record external outages or unavailable tests explicitly. A failed server starts root-cause investigation and does not authorize a server-specific workaround. Follow `docs/validation.md` and `docs/server-targets.md` for the report workflow, endpoint references, and sensitive-data boundary.
